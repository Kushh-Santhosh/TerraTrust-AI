...
function coerceResult(raw: unknown, p: Property): VerificationResult {
  ...
  const status = r.status ?? (r.decision === "VERIFIED" ? "verified" : r.decision === "HUMAN_REVIEW_REQUIRED" ? "manual_review" : "rejected");
  return {
...
export async function runVerification(p: Property, signal?: AbortSignal): Promise<RunOutcome> {
  ...
  try {
    ...
    return { result: computeVerification(p, "demo"), fallbackReason: `Demo simulation — live n8n verification unavailable (${reason}).` };
  }
}
