...
function GovernmentPage() {
  ...
  if (pathname !== "/government") return <Outlet />;
  return (
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_400px]">
        ...
        <div className="surface-card flex flex-col p-5">
          ...
          <ul className="mt-3 divide-y divide-border">
            {properties.filter(p => p.status === "disputed").concat(properties.slice(1,2)).filter(p => !resolved.includes(p.id)).map(p => (
              <li key={p.id} className="py-3">
                <p className="text-sm font-medium">{p.title}</p>
                <p className="text-xs text-muted-foreground">{p.passportId} · {p.region}</p>
                <div className="mt-2 flex gap-2">
                  <Link to="/properties/$id/verify" params={{ id: p.id }}><Button size="sm" variant="outline">Investigate</Button></Link>
                  <Button size="sm" onClick={() => setResolved(items => [...items, p.id])}>Resolve</Button>
                </div>
              </li>
            ))}
          </ul>
