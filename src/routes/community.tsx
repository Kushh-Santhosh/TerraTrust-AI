        ...
        {items.map(p => (
          <div key={p.passport} className="surface-card flex flex-col gap-3 p-5">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setResponses(r => ({ ...r, [p.passport]: "Attestation submitted" }))}><ThumbsUp className="h-4 w-4" /> {responses[p.passport] === "Attestation submitted" ? responses[p.passport] : "Yes, attest"}</Button>
              <Button variant="outline" className="flex-1" onClick={() => setResponses(r => ({ ...r, [p.passport]: "Objection submitted" }))}><ThumbsDown className="h-4 w-4" /> {responses[p.passport] === "Objection submitted" ? responses[p.passport] : "Dispute"}</Button>
            </div>
