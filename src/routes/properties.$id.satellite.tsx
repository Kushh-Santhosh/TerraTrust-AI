      ...
      actions={
        <>
          <Button variant="outline">
            <Calendar className="h-4 w-4" /> Compare dates
          </Button>
          <Button>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
        <div className="surface-card relative overflow-hidden p-0">
          ...
          <div className="absolute right-4 top-4 flex gap-2">
            <Pill tone="info">
              <Layers className="h-3 w-3" /> Sentinel-2 · 2024-09-01
            </Pill>
          </div>
        </div>
        <div className="space-y-3">
          ...
          {snapshots.map((s) => (
            <button
              key={s.date}
              className="surface-card w-full p-3 text-left transition hover:shadow-[var(--shadow-elev)]"
            >
