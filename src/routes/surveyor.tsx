...
function SurveyorPage() {
  ...
  return (
    <AppShell title="Surveyor workspace" subtitle="Assignments, boundary captures, and quality scoring."
      actions={<Link to="/surveyor/assignments"><Button className="rounded-full"><Briefcase className="h-4 w-4" /> View assignments</Button></Link>}>
        ...
        <div className="surface-card p-5">
          ...
          <ul className="mt-4 space-y-3">
            {["Kaduna Farmland — 14 boundary points","Ibadan Compound — 8 boundary points","Wuse II Commercial — 12 boundary points"].map(x => (
              <li key={x} className="flex items-center justify-between rounded-lg border border-border p-3 text-sm">{x}<Link to="/surveyor/assignments/$id" params={{ id: "S-2238" }}><Button size="sm" variant="ghost">Review</Button></Link></li>
            ))}
          </ul>
        </div>
      </div>
