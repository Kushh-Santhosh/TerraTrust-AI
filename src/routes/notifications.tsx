...
function NotificationsPage() {
  ...
  return (
    <AppShell title="Notifications" subtitle="Updates from your portfolio, community, and government registries."
      actions={<Button variant="outline" className="rounded-full" onClick={() => setAllRead(true)}>Mark all read</Button>}>
      <div className="surface-card divide-y divide-border">
        {notifications.map(n => (
          <div key={n.id} className={`flex items-start gap-4 p-5 ${!n.read && !allRead ? "bg-primary/[0.03]" : ""}`}>
